...
public:
	...
	optional_ptr<ParseResult> MatchParseResult(MatchState &state) const override {
		...
		if (IsQuoted(result_text)) {
			result_text = PEGKeywordHelper::TryUnescapeQuotes(result_text);
	...
	SuggestionType AddSuggestionInternal(MatchState &state) const override {
		MatcherSuggestion sugg(suggestion_type);
		FillMatcherQualifierContext(state.tokens, suggestion_type, sugg);
		state.AddSuggestion(std::move(sugg));
		return SuggestionType::MANDATORY;
	}
...
shared_ptr<PEGMatcher> PEGMatcherCache::GetMatcher() {
	...
	new_matcher->from_clause_root = factory.CreateMatcher(grammar_string.c_str(), "FromClause");
	new_matcher->with_clause_root = factory.CreateMatcher(grammar_string.c_str(), "WithClause");
#else
	new_matcher->root = factory.CreateMatcher(const_char_ptr_cast(INLINED_PEG_GRAMMAR), "Statement");
	new_matcher->from_clause_root = factory.CreateMatcher(const_char_ptr_cast(INLINED_PEG_GRAMMAR), "FromClause");
	new_matcher->with_clause_root = factory.CreateMatcher(const_char_ptr_cast(INLINED_PEG_GRAMMAR), "WithClause");
#endif
	std::unique_lock<std::mutex> lock(mutex);
	if (!matcher) {
